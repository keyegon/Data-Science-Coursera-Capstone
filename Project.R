mergeFiles <- function() {
  # if (!file.exists("final/en_US/mergedfile.txt")) {
  conn <- file("final/en_US/mergedfile.txt", "r")
  fulltext <- readLines(conn, encoding = "UTF-8", skipNul = TRUE)
  nlines <- length(fulltext)
  close(conn)
  
  conn <- file("final/en_US/sampledfile.txt", "w")
...
makeCorpus <- function() {
  mytf3 <-
    readLines("final/en_US/sampledfile.txt",
